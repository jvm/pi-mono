...
Set a stable path to this skill CLI. Resolve `scripts/image_gen.py` relative to this skill directory:

```bash
export IMAGE_GEN="scripts/image_gen.py"
```

Install dependencies into the active Python environment with its package manager. In uv-managed environments, `uv pip install ...` remains the preferred path.
...
- Do not silently downgrade from CLI `gpt-image-2` or Pi `codex_generate_image` to CLI `gpt-image-1.5`; ask first unless the user already explicitly requested `gpt-image-1.5`, `scripts/image_gen.py`, or CLI fallback.
